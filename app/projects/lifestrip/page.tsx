...
export default function LifeStripPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const selectedMemberDetails = family.filter(m => selectedMembers.includes(m.id));
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6 border-b-2 border-[#E5E1DA] pb-10">
          <div>
            <div className="flex items-center gap-2 text-[#92C7CF] mb-3 font-bold">
              <FaBookOpen />
              <span className="text-xs uppercase tracking-[0.2em] font-black">Digital Serialization</span>
            </div>
          ...
          <nav className="flex gap-4">
            <Link href="/projects/lifestrip/family" className="flex items-center gap-2 px-5 py-2.5 bg-white border border-[#E5E1DA] rounded-2xl text-slate-500 hover:text-[#92C7CF] hover:border-[#AAD7D9] hover:bg-[#FBF9F1] transition-all font-bold text-xs uppercase tracking-wider shadow-sm">
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <section className="lg:col-span-12 xl:col-span-5 space-y-8">
            <div className="bg-white border-2 border-[#E5E1DA] rounded-[2.5rem] p-10 shadow-xl shadow-[#E5E1DA]/20 relative overflow-hidden group">
              ...
              <div className="pt-8 border-t border-[#E5E1DA] space-y-6">
                 ...
                 <div className="flex flex-wrap gap-2.5">
                   ...
                   {family.length === 0 && (
                     <Link href="/projects/lifestrip/family" className="text-xs text-[#92C7CF] font-bold hover:underline">Add family members first</Link>
                   )}
                 </div>
              </div>
              ...
              >
                {isGenerating ? (
                  <>
                    ...
                    <span className="tracking-widest text-sm text-white/80">Inking Diary...</span>
                  </>
                ) : (
                  <>
            ...
            <p className="text-slate-300 font-bold text-[9px] text-center px-4 leading-relaxed uppercase tracking-[0.25em]">
              Powered by <span className="text-[#92C7CF]">Nano Banana</span> • Character Identity Synchronization
            </p>
          </section>
          ...
          <section className="lg:col-span-12 xl:col-span-7">
            {panels.length > 0 ? (
              <div className="grid grid-cols-2 gap-8 p-10 bg-white border-2 border-[#E5E1DA] rounded-[3rem] shadow-2xl shadow-[#E5E1DA]/10">
                {panels.map((url, i) => (
                  <div key={i} className="aspect-square bg-[#FBF9F1] rounded-[2rem] overflow-hidden relative group hover:scale-[1.03] transition-all duration-500 border-2 border-[#E5E1DA]">
              ...
              <div className="aspect-[4/5] bg-white border-2 border-[#E5E1DA] rounded-[3rem] shadow-inner flex flex-col items-center justify-center text-slate-200 p-12 text-center relative overflow-hidden group">
                ...
                <h3 className="text-3xl font-black mb-4 tracking-tighter text-slate-800">New Storyline</h3>
                <p className="text-[11px] max-w-sm uppercase tracking-[0.2em] text-slate-400 font-bold leading-relaxed">Your comic is currently blank. Pen your thoughts to generate the 4-panel series.</p>
              </div>
            )}
          </section>
        </main>
